    ...
    def get_daily_data(
        ...
        try:
            ...
            if raw_df.empty:
                ...
                logger.info(f"[{self.name}] {stock_code} returned empty daily data: range={start_date} ~ {end_date}, elapsed={elapsed:.2f}s")
            ...
            elapsed = time.time() - request_start
            logger.info(f"[{self.name}] {stock_code} successful: range={start_date} ~ {end_date}, rows={len(df)}, elapsed={elapsed:.2f}s")
            return df

        except Exception as e:
            elapsed = time.time() - request_start
            error_type, error_reason = summarize_exception(e)
            logger.error(f"[{self.name}] {stock_code} failed: range={start_date} ~ {end_date}, error_type={error_type}, elapsed={elapsed:.2f}s, reason={error_reason}")
...
class DataFetcherManager:
    ...
    @classmethod
    def _filter_daily_fetchers_for_market(cls, fetchers: List[BaseFetcher], market: str) -> List[BaseFetcher]:
        ...
        for fetcher in fetchers:
            ...
            if supported is not None and market not in supported:
                ...
                kept.append(fetcher)
        if skipped:
            logger.info("[Data source routing] %s daily skip unsupported sources: %s", market, ", ".join(skipped))
        return kept

    @classmethod
    def _filter_fetchers_by_capability(cls, fetchers: List[BaseFetcher], capability: str) -> List[BaseFetcher]:
        kept: List[BaseFetcher] = []
        skipped: List[str] = []
        for fetcher in fetchers:
            if cls._is_fetcher_available(fetcher, capability=capability):
                ...
                skipped.append(fetcher.name)
        if skipped:
            logger.info("[Data source routing] %s skip temporarily unavailable sources: %s", capability or "request", ", ".join(skipped))
        return kept
    ...
    @classmethod
    def _is_daily_source_available(cls, fetcher: BaseFetcher, market: str) -> bool:
        key = cls._daily_health_key(fetcher, market)
        if cls._daily_source_health.is_available(key):
            return True
        logger.info("[Data source health] %s daily skip temporarily melted source: %s", market, fetcher.name)
        return False
    ...
    def prefetch_realtime_quotes(self, stock_codes: List[str]) -> int:
        ...
        if first_bulk_source_index is None or first_bulk_source_index >= 2:
            logger.info("[Prefetch] component=realtime_prefetch action=skip reason=no_early_bulk_source priority=%s", priority)
            return 0

        if len(stock_codes) < 5:
            logger.info("[Prefetch] component=realtime_prefetch action=skip reason=small_batch stock_count=%d threshold=5 bulk_source=%s", len(stock_codes), priority_list[first_bulk_source_index])
            return 0

        bulk_source = priority_list[first_bulk_source_index]
        logger.info("[Prefetch] component=realtime_prefetch action=start stock_count=%d bulk_source=%s first_code=%s", len(stock_codes), bulk_source, stock_codes[0])

        try:
            first_code = stock_codes[0]
            quote = self.get_realtime_quote(first_code)
            if quote:
                logger.info("[Prefetch] component=realtime_prefetch action=complete status=success stock_count=%d bulk_source=%s", len(stock_codes), bulk_source)
                return len(stock_codes)
            else:
                logger.warning("[Prefetch] component=realtime_prefetch action=complete status=failed stock_count=%d bulk_source=%s fallback=per_stock", len(stock_codes), bulk_source)
                return 0
        except Exception as e:
            logger.error("[Prefetch] component=realtime_prefetch action=complete status=error stock_count=%d bulk_source=%s error=%s", len(stock_codes), bulk_source, e)
            return 0
    ...
    def _enrich_realtime_quote(self, quote, *, fallback_from: Optional[str] = None, realtime_cache_ttl: Optional[int] = None):
        if quote is None:
            return None
        ...
        provider_dt = self._parse_realtime_timestamp(getattr(quote, "provider_timestamp", None))
    ...
    def get_realtime_quote(self, stock_code: str, *, log_final_failure: bool = True):
        ...
        if is_jp or is_kr:
            ...
            if quote is not None:
                ...
                return self._enrich_realtime_quote(quote, realtime_cache_ttl=getattr(config, "realtime_cache_ttl", None))
        ...
        if is_us or is_hk:
            ...
            primary_quote = self._supplement_quote(stock_code, primary_quote, secondary_src, **secondary_kw)
            ...
            if primary_quote is not None:
                return self._enrich_realtime_quote(primary_quote, fallback_from=fallback_from, realtime_cache_ttl=getattr(config, "realtime_cache_ttl", None))
            if log_final_failure:
                logger.info(f"[Realtime quote] {market_label} {stock_code} no available source")
            return None
    ...
    def _longbridge_preferred(self, capability: str = "realtime_quote") -> bool:
        return self._get_fetcher_by_name("LongbridgeFetcher", capability=capability) is not None
        ...
        for index, (fetcher, fetcher_name, source_key) in enumerate(candidate_fetchers):
            ...
            try:
                ...
                if _is_meaningful_chip_distribution(chip):
                    ...
                    if chip is not None:
                        logger.warning("[Chip distribution] %s returned incomplete or placeholder, trying next", fetcher_name)
